# Patrick Evers — Personal Portfolio

A personal portfolio and resume website built with Vue.js, TypeScript, and TailwindCSS 3.

## Data Source

All portfolio content (personal info, experience, education, skills, projects, certifications, memberships) is **scraped and parsed at build time from `ATS_RESUME.md`** — an ATS-style plain Markdown resume file in the project root.

- The store `src/stores/markdownResumeStore.ts` imports `ATS_RESUME.md` as raw text via Vite's `?raw` import and parses it into the `Resume` typed data model (`src/data/resume.ts`).
- A US variant is loaded from `ATS_US_RESUME.md` via `src/stores/usMarkdownResumeStore.ts`.
- The fallback static data in `src/data/resume.ts` (`resumeData`) is only used if the Markdown store fails to parse.
- **To update portfolio content, edit `ATS_RESUME.md` directly** — no component or store changes are needed for content updates.
...
```
ATS_RESUME.md           # Primary content source — ATS-style resume (edit this for content)
ATS_US_RESUME.md        # US-variant resume
